-- 用户表
CREATE TABLE IF NOT EXISTS yesand.users (
...
CREATE TABLE IF NOT EXISTS yesand.chat_sessions (
    ...
    CONSTRAINT chat_sessions_session_scope_check CHECK (session_scope::text = ANY (ARRAY['anonymous'::character varying, 'authenticated'::character varying]::text[])),
...
CREATE INDEX IF NOT EXISTS idx_chat_sessions_user_id_scope ON yesand.chat_sessions (user_id, session_scope);
...
CREATE INDEX IF NOT EXISTS idx_subscriptions_pay_subscription_id ON yesand.subscriptions (pay_subscription_id);
CREATE INDEX IF NOT EXISTS idx_subscriptions_order_id ON yesand.subscriptions (order_id);
CREATE INDEX IF NOT EXISTS idx_subscriptions_user_id ON yesand.subscriptions (user_id);

-- 积分表
CREATE TABLE IF NOT EXISTS yesand.credits (
    id                        BIGSERIAL PRIMARY KEY,
    user_id                   UUID        NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_credits_user_id ON yesand.credits (user_id);

-- 交易订单表
CREATE TABLE IF NOT EXISTS yesand.transactions (
    id                   BIGSERIAL PRIMARY KEY,
    user_id              UUID         NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_transactions_order_id ON yesand.transactions (order_id);
CREATE INDEX IF NOT EXISTS idx_transactions_pay_subscription_id ON yesand.transactions (pay_subscription_id);
CREATE INDEX IF NOT EXISTS idx_transactions_user_id ON yesand.transactions (user_id);


-- 积分使用审计表
CREATE TABLE IF NOT EXISTS yesand.credit_audit_log (
    id               BIGSERIAL PRIMARY KEY,
    user_id          UUID         NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_credit_audit_log_credit_type ON yesand.credit_audit_log (credit_type);
CREATE INDEX IF NOT EXISTS idx_credit_audit_log_operation_type ON yesand.credit_audit_log (operation_type);
CREATE INDEX IF NOT EXISTS idx_credit_audit_log_user_id ON yesand.credit_audit_log (user_id);


-- 用户信息备份表
CREATE TABLE IF NOT EXISTS yesand.user_backup (
    id                BIGSERIAL PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_user_backup_clerk_user_id ON yesand.user_backup (clerk_user_id);
CREATE INDEX IF NOT EXISTS idx_user_backup_email ON yesand.user_backup (email);
CREATE INDEX IF NOT EXISTS idx_user_backup_fingerprint_id ON yesand.user_backup (fingerprint_id);
CREATE INDEX IF NOT EXISTS idx_user_backup_original_user_id ON yesand.user_backup (original_user_id);


-- 第三方对接日志审计表
CREATE TABLE IF NOT EXISTS yesand.apilog (
    id            BIGSERIAL PRIMARY KEY,
